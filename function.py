...
def criteria(p, ma, dr):
    ...
    if (discripency_rate_cr <0.1 and annualized_growth_rate > 0) :
        return True
    else:
        return False
...
def criteria3(p, ma_high_freq, ma_low_freq, dr):
    ...
    if p[-1]<ma_high_freq[-1] and discripency_rate_cr>0.1:
        return True
    else:
        return False
...
def backtest(name, country='kr'):
    budget = 100
    no_share = 0
    bs = 0 # if bs = 0: sell, bs = 1: buy
    holding_period = 0
    backtest_start = date(2013, 8, 14)
    backtest_end = date(2020, 11, 11)
    ...
    p, ma_240 = ma(l, start_index, 240)
    _, ma_60 = ma(l, start_index, 60)
    dr = derivative(ma_240)
    x = x[start_index:]
